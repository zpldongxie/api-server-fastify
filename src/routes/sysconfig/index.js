 ...
 * @LastEditTime: 2020-09-15 17:39:39
 * @LastEditors: zpl
 */
...
const routerBaseInfo = {
  modelName_U: 'SysConfig',
  modelName_L: 'sysconfig',
  getURL: '/api/sysconfig/:id',
  getAllURL: '/api/sysconfigs',
  getListURL: '/api/getSysConfigList',
  putURL: '/api/sysconfig',
  deleteURL: '/api/sysconfigs',
};
module.exports = fp(async (server, opts, next) => {
  ...
  server.get(
      ...
      { schema: { ...getByIdSchema, tags: ['sysconfig'], summary: '根据ID获取单个' } },
      async (request, reply) => {
        const runFun = async () => {
  ...
  server.get(
      ...
      { schema: { tags: ['sysconfig'], summary: '获取所有' } },
      async (request, reply) => {
        const runFun = async () => {
          const conditions = {};
  ...
  server.post(
      ...
      { schema: { ...queryListSchema, tags: ['sysconfig'], summary: '根据条件获取列表' } },
      async (request, reply) => {
  ...
  server.put(routerBaseInfo.putURL,
      { schema: { ...updateSchema, tags: ['sysconfig'], summary: '新增或更新' } },
      async (request, reply) => {
      // 参数校验
  ...
  server.delete(
      ...
      { schema: { ...deleteSchema, tags: ['sysconfig'], summary: '批量删除' } },
      async (request, reply) => {
