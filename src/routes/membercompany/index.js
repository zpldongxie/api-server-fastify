 ...
 * @LastEditTime: 2020-09-15 10:07:55
 * @LastEditors: zpl
 */
...
module.exports = fp(async (server, opts, next) => {
  ...
  server.get(
      ...
      { schema: { ...getByIdSchema, tags: ['membercompany'], description: '根据ID获取单个企业会员' } },
      async (request, reply) => {
        const runFun = async () => {
  ...
  server.get(
      ...
      { schema: { tags: ['membercompany'], description: '获取所有企业会员' } },
      async (request, reply) => {
        const runFun = async () => {
          const conditions = {};
  ...
  server.post(
      ...
      { schema: { ...queryListSchema, tags: ['membercompany'], description: '根据条件获取企业会员列表' } },
      async (request, reply) => {
  ...
  server.put(routerBaseInfo.putURL,
      { schema: { ...updateSchema, tags: ['membercompany'], description: '新增或更新企业会员' } },
      async (request, reply) => {
      // 参数校验
  ...
  server.delete(
      ...
      { schema: { ...deleteSchema, tags: ['membercompany'], description: '批量删除企业会员' } },
      async (request, reply) => {
