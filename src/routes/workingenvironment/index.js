...
const routerBaseInfo = {
  modelName_U: 'WorkingEnvironment',
  modelName_L: 'workingenvironment',
  getURL: '/api/workingenvironment/:id',
  getAllURL: '/api/workingenvironments',
  getListURL: '/api/getWorkingEnvironmentList',
  putURL: '/api/workingenvironment',
...
module.exports = fp(async (server, opts, next) => {
  ...
  */
  ...
  server.post(
      ...
      { schema: { ...queryListSchema, tags: ['workingenvironment'], summary: '根据条件获取列表' } },
      (request, reply) => method.queryList(request, reply),
  );

  const updateSchema = require('./update-schema');
  server.put(routerBaseInfo.putURL,
      { schema: { ...updateSchema, tags: ['workingenvironment'], summary: '新增或更新' } },
