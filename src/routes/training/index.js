 ...
 * @LastEditTime: 2020-09-15 17:39:30
 * @LastEditors: zpl
 */
...
module.exports = fp(async (server, opts, next) => {
  ...
  server.get(
      ...
      { ...getByIdSchema, schema: { tags: ['training'], summary: '根据ID获取单个培训' } },
      async (request, reply) => {
        const runFun = async () => {
  ...
  server.get(
      ...
      { schema: { tags: ['training'], summary: '获取所有培训' } },
      async (request, reply) => {
        const runFun = async () => {
          const conditions = {};
  ...
  server.post(
      ...
      { schema: { ...queryListSchema, tags: ['training'], summary: '根据条件获取培训列表' } },
      async (request, reply) => {
  ...
  server.put(routerBaseInfo.putURL,
      { schema: { ...updateSchema, tags: ['training'], summary: '新增或更新培训' } },
      async (request, reply) => {
      // 参数校验
  ...
  server.delete(
      ...
      { schema: { ...deleteSchema, tags: ['training'], summary: '批量删除培训' } },
      async (request, reply) => {
