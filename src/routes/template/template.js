 ...
 * @LastEditTime: 2020-09-15 10:15:38
 * @LastEditors: zpl
 */
...
const routerBaseInfo = {
  modelName_U: 'CurrentModelName',
  modelName_L: 'currentModelName',
  getURL: '/api/currentModelName/:id',
  getAllURL: '/api/currentModelNames',
  getListURL: '/api/getCurrentModelNameList',
  putURL: '/api/currentModelName',
  deleteURL: '/api/currentModelNames',
};
module.exports = fp(async (server, opts, next) => {
  ...
  server.get(
      ...
      { schema: { ...getByIdSchema, tags: ['currentModelName'], description: '根据ID获取单个' } },
      async (request, reply) => {
        const runFun = async () => {
  ...
  server.get(
      ...
      { schema: { tags: ['currentModelName'], description: '获取所有' } },
      async (request, reply) => {
        const runFun = async () => {
          const conditions = {};
  ...
  server.post(
      ...
      { schema: { ...queryListSchema, tags: ['currentModelName'], description: '根据条件获取列表' } },
      async (request, reply) => {
  ...
  server.put(routerBaseInfo.putURL,
      { schema: { ...updateSchema, tags: ['currentModelName'], description: '新增或更新' } },
      async (request, reply) => {
      // 参数校验
  ...
  server.delete(
      ...
      { schema: { ...deleteSchema, tags: ['currentModelName'], description: '批量删除' } },
      async (request, reply) => {
